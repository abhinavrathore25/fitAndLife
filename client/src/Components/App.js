...
import { useHorizontalScroll } from './CustomHooks/SideScroll';
...
function App() {
  ...
  const handleLogin = (login, showModal) => {
    setIsLoggedIn(login);
  ...
  return (
    <>
      <div id='header'>
        <Navbar showLogin={showLogin} setShowLogin={(value) => setShowLogin(value)} />
      </div>
      {!isLoggedIn && <>
      ...
      {showLogin && <Login isLoggedIn={handleLogin} />}
      {isLoggedIn && <AdminScreen />}
    </>
  );
}
